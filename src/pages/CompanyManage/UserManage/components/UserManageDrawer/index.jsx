...
function UserManageDrawer(props, ref) {
  ...
  const headRows = record.length > 4 ? record.slice(0, 4) : record;
  const checkNum = record.length;

  const headLen = headRows.length

  const avatarBoxWidth = useMemo(() => {
    return headLen * 32 - (headLen - 1) * 12
  },[headLen])

  return (
    <Drawer
      className={styles.drawerPro}
      title="编辑信息"
      loading={listLoading}
      closable={false}
      open={open}
      ...
      <div className={styles.container}>
        ...
        {type === "batch" ? (
          <div className={classNames(styles.selectedUsers, styles.radioGroup)}>
            <div>选中了</div>
            <div className={styles.avatarBox} style={{width:`${avatarBoxWidth}px`}}>
