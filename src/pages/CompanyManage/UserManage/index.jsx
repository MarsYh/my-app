...
function UserManage() {
  ...
  function renderEdit(record, key) {
    ...
    const _edit = `${key}_${record.uuid}`;
    return (
      <Space
        onClick={() =>
          key === "inName"
            ? useManageModalRef.current.open(record)
            : userManageDrawerRef.current.open(record, key)
        }
        className={styles.inName}
        onMouseEnter={() => setEdit(_edit)}
      ...
      >
        ...
        <EditOutlined
          ...
          style={{
            visibility: edit === _edit ? "visible" : "hidden",
          }}
        />
      </Space>
    );
  ...
  return (
    <div className={styles.container}>
      ...
      <UserManageModal ref={useManageModalRef} onSuccess={onEditSuccess}/>
